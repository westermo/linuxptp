...
static const char* transport2str(enum transport_type type) {
	switch (type) {
	case TRANS_UDS:
		return "UDS";
	case TRANS_UDP_IPV4:
...
static void pmc_show(struct ptp_message *msg, FILE *fp)
{
	...
	case MID_DEFAULT_DATA_SET:
		...
		if (mgt->length == HDR_SIZE + sizeof(struct defaultDS)) {
			fprintf(fp,
				IFMT "profileSet              %s"
	...
	case MID_TRANSPARENT_CLOCK_DEFAULT_DATA_SET:
		...
		/* If we received the full extended IEC 62439-3 dataset */
		if (mgt->length == HDR_SIZE + sizeof(struct transparentClockDefaultDS)) {
			fprintf(fp,
				IFMT "profileSet              %s"
				IFMT "timeInaccuracy          %f",
				profile_set2str(tcds->iec62439_ds.profileSet),
				(float)tcds->iec62439_ds.timeInaccuracy / (1<<16));
		}
		break;
	...
	case MID_PORT_DATA_SET:
		...
		/* If we received the full extended IEC 62439-3 dataset */
		if (mgt->length == HDR_SIZE + sizeof(struct portDS)) {
			fprintf(fp,
				IFMT "networkTransport        %s"
				IFMT "portEnabled             %s"
				IFMT "dlyAsymmetry            %" PRIi64
				IFMT "profileId               %s"
				IFMT "vlanEnable              %s"
				IFMT "vlanId                  %u"
				IFMT "vlanPrio                %u"
				...
				IFMT "prpPairedPort           %hu"
				IFMT "errorCounter            %u"
				IFMT "peerDelayLim            %" PRIi64,
				transport2str(p->iec62439_ds.networkProtocol),
	...
	case MID_TRANSPARENT_CLOCK_PORT_DATA_SET:
		...
		/* If we received the full extended IEC 62439-3 dataset */
		if (mgt->length == HDR_SIZE + sizeof(struct transparentClockPortDS)) {
			fprintf(fp,
				IFMT "portEnabled             %s"
				IFMT "dlyAsymmetry            %" PRIi64
				IFMT "twoStepFlag             %s"
