...
static enum fsm_event red_port_link_status_event(struct red_port *rp)
{
	...
	if (rp_up) {
		...
		if (other->state == PS_FAULTY) {
			...
			return red_switchover(rp, PS_FAULTY);
		}
	}
	return EV_NONE;
}
...
static void red_switch_phc(struct port *p, int phc_index)
{
	if (p->phc_index == phc_index)
		return;
	// TODO: ensure phc_index is set correctly
...
/* static void red_port_faulty_dispatch(struct red_port *rp) */
/* { */
/* 	struct red_port *other = red_other_port(rp); */

/* 	if (rp->state == PS_FAULTY && other->state == PS_FAULTY) */
/* 		rp->upper->dispatch(rp->upper, EV_FAULT_DETECTED, 0); */
/* } */
...
static int red_tx_sync(struct port *p, struct address *dst, uint16_t sequence_id)
{
	...
	// TODO: 2-step not supported. Using TS_SOFTWARE used for testing
	...
	err = red_port_prepare_and_send(red_get_active_port(p), msg, event);
	if (err) {
...
static int red_tx_announce(struct port *p, struct address *dst, uint16_t sequence_id)
{
	...
	err = red_port_prepare_and_send(red_get_active_port(p), msg, TRANS_GENERAL);
	if (err) {
...
static int red_port_pdelay_request(struct red_port *rp)
{
	...
	msg->header.sourcePortIdentity = rp->upper->portIdentity;
...
static int red_port_process_pdelay_req(struct red_port *rp, struct ptp_message *m)
{
	...
	rsp->header.sourcePortIdentity = rp->upper->portIdentity;
...
static void red_port_peer_delay(struct red_port *rp)
{
	...
	if (!pid_eq(&rsp->pdelay_resp.requestingPortIdentity, &rp->upper->portIdentity))
		return;
	...
	if (!pid_eq(&fup->pdelay_resp_fup.requestingPortIdentity, &rp->upper->portIdentity))
		return;
...
static enum fsm_event red_event(struct port *p, int fd_index)
{
	...
	case MANAGEMENT:
		// TODO
		/* if (red_port_clock_manage(p->clock, rp, msg)) */
			/* event = EV_STATE_DECISION_EVENT; */
		break;
	}
...
{
	pr_err("casan %s %s: %d %d", __func__, rp->log_name, rp->phc_index, interface_phc_index(rp->iface));
